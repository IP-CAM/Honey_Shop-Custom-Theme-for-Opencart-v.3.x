...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
            ...
            $data['weight']=(double)$product_info['weight'];
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'],700,700)
				);
			}
			
			$data['newstart'] = $product_info['date_added'];
			$data['val1'] = preg_replace("/[^0-9.]/", "", $product_info['special']);
			$data['val2'] = preg_replace("/[^0-9.]/", "", $product_info['price']);
			$data['startDate1'] = strtotime(mb_substr($product_info['date_added'], 0, 10));
			$data['endDate2'] = strtotime(date("Y-m-d"));
                ...
                foreach ($option['product_option_value'] as $option_value) {
                    if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
                        ...
                        if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
                            $price = (int)$data['price'] + (int)$option_value['price'];
                            if($data['type']=='кг'){
                                $counter=$data['weight']*1000;
                            }
                            else{
                                $counter=$data['weight'];
                            }
                            $weight = $counter + $option_value['weight'];
                            if($weight>1000){
                                $_weight=$weight/1000;
                                $_weight_class='кг';
                            }
